...
% tau = 1;dt = 0.001;
% for i = 1:6
%     dmp(i) = dmp(i).LWR(tau,demoTraj_jtraj{i});
%     % Run it once for the next step to see the performance (Not necessary)
%     [dmpTraj{i},x{i},fx{i}] = dmp(i).run(demoTraj_jtraj{i}(1,1),demoTraj_jtraj{i}(end,1),tau,dt); 
% end
...
% dmp(i).plotCompare(dmpTraj{i},demoTraj_jtraj{i},tau);
% clear i
