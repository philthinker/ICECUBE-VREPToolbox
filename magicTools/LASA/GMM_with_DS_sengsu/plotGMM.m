...
nbData = size(Mu,2);
lightcolor = color + [0.6,0.6,0.6]*5/3; %remove *5/3
...
if display_mode==1
  ...
  for j=1:nbData
    stdev = sqrtm(Sigma(:,:,j));
    X = [cos(t) sin(t)] * real(2.*stdev) + repmat(Mu(:,j)',nbDrawingSeg,1);
    patch(X(:,1), X(:,2), lightcolor, 'lineWidth', 1.5, 'EdgeColor', [1 0 0], 'FaceColor', [1 0.9 0.9]);% 'AlphaDataMapping' , 'none', 'FaceVertexAlphaData', 0); %linewidth=2
...
elseif display_mode==2
  ...
  t = linspace(-pi, pi, nbDrawingSeg)';
  for j=1:nbData
    stdev = sqrtm(1.0.*Sigma(:,:,j)); %1.0->3.0
