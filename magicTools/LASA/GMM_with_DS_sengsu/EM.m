function [Priors, Mu, Sigma, nbStep, Pix] = EM(Data, Priors0, Mu0, Sigma0)
%
...
while 1
  ...
  if (~isempty(find(sum(Pix_tmp,2) < 1E-15)))
    Pix_tmp = Pix_tmp + repmat(1E-15, size(Pix_tmp, 1), size(Pix_tmp, 2));
  end;
%   
  Pix = Pix_tmp ./ repmat(sum(Pix_tmp,2),[1 nbStates]);
  %Compute cumulated posterior probability
  E = sum(Pix);
  if (~isempty(find(sum(E,2) < 1E-15)))
      E(find(sum(E,2) < 1E-15)) = repmat(1E-15, 1, size(find(sum(E,2) < 1E-15)));
  end;
  %% M-step %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  for i=1:nbStates
    %Update the priors
    ...
    Sigma(:,:,i) = Sigma(:,:,i) + 1E-8.*diag(ones(nbVar,1)); %%%note
  end
  %% Stopping criterion %%%%%%%%%%%%%%%%%%%%
  for i=1:nbStates
